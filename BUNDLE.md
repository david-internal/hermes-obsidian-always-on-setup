...
- `codex-goal-syncthing-vault-sync.md`: a video-ready Codex `/goal` prompt for adding Syncthing as an optional vault sync path.
...
4. Use `codex-goal-syncthing-vault-sync.md` in the video if you want Codex to add a Syncthing alternative.
5. Use the skill with an agent when the viewer wants guided setup or gets stuck.
6. Use the troubleshooting reference if notes are created but do not appear on the Mac.
